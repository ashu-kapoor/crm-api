...
} = require("../controllers/getAttachmentsController");

module.exports.getAttachments = (app, middlewares, validator = null) => {
  ...
  if (middlewares && lodash.isArray(middlewares)) {
    middlewares.forEach((middleware) => {
      if (lodash.isFunction(middleware)) {
        ...
        throw new Error("Middleware not function " + getAttachmentsRoute);
      }
    });
  }
  ...
  if (validator && lodash.isFunction(validator)) {
    ...
    throw new Error("Validator not function " + getAttachmentsRoute);
  }
  middleWaresList.push(getAttachmentsController);
  app.get("/crm/v1/contacts/:contactId/attachments", middleWaresList);
};
