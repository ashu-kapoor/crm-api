...
} = require("../controllers/createContactController");

module.exports.createContact = (app, middlewares, validator = null) => {
  ...
  if (middlewares && lodash.isArray(middlewares)) {
    middlewares.forEach((middleware) => {
      if (lodash.isFunction(middleware)) {
        ...
        throw new Error("Middleware not function " + createContactRoute);
      }
    });
  }
  ...
  if (validator && lodash.isFunction(validator)) {
    ...
    throw new Error("Validator not function " + createContactRoute);
  }
  middleWaresList.push(createContactController);
  app.post("/crm/v1/contacts", middleWaresList);
};
