...
} = require("../controllers/createOpportunityController");
const lodash = require("lodash");

module.exports.createOpportunity = (app, middlewares, validator = null) => {
  ...
  if (middlewares && lodash.isArray(middlewares)) {
    middlewares.forEach((middleware) => {
      if (lodash.isFunction(middleware)) {
        ...
        throw new Error("Middleware not function " + createOpportunityRoute);
      }
    });
  }
  ...
  if (validator && lodash.isFunction(validator)) {
    ...
    throw new Error("Validator not function " + createOpportunityRoute);
  }
  middleWaresList.push(createOpportunityController);
  app.post("/crm/v1/contacts/:contactId/opportunities", middleWaresList);
};
