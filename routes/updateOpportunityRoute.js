...
} = require("../controllers/updateOpportunityController");
const lodash = require("lodash");

module.exports.updateOpportunity = (app, middlewares, validator = null) => {
  ...
  if (middlewares && lodash.isArray(middlewares)) {
    middlewares.forEach((middleware) => {
      if (lodash.isFunction(middleware)) {
        ...
        throw new Error("Middleware not function " + updateOpportunityRoute);
      }
    });
  }
  ...
  if (validator && lodash.isFunction(validator)) {
    ...
    throw new Error("Validator not function " + updateOpportunityRoute);
  }
  middleWaresList.push(updateOpportunityController);
  app.put(
    "/crm/v1/contacts/:contactId/opportunities/:opportunityId",
    middleWaresList
  );
};
