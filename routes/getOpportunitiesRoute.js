...
} = require("../controllers/getOpportunitiesController");

module.exports.getOpportunities = (app, middlewares, validator = null) => {
  ...
  if (middlewares && lodash.isArray(middlewares)) {
    middlewares.forEach((middleware) => {
      if (lodash.isFunction(middleware)) {
        ...
        throw new Error("Middleware not function " + getOpportunitiesRoute);
      }
    });
  }
  ...
  if (validator && lodash.isFunction(validator)) {
    ...
    throw new Error("Validator not function " + getOpportunitiesRoute);
  }
  middleWaresList.push(getOpportunitiesController);
  app.get("/crm/v1/contacts/:contactId/opportunities", middleWaresList);
};
